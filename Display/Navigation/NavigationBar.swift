...
public final class NavigationBarTheme {
    public static func generateBackArrowImage(color: UIColor) -> UIImage? {
        return generateImage(CGSize(width: 13.0, height: 22.0), rotatedContext: { size, context in
            ...
            context.translateBy(x: 0.0, y: -CCScreenPixel)
...
open class NavigationBar: ASDisplayNode {
    ...
    private var title: String? {
        didSet {
            if let title = self.title {
                self.titleNode.attributedText = NSAttributedString(string: title, font: Font.bold(21.0), textColor: self.presentationData.theme.primaryTextColor)
    ...
    private var lockClickableNode: ASDisplayNode
    @objc private func navigationTap() {
    ...
    public init(presentationData: NavigationBarPresentationData) {
        ...
        self.lockClickableNode = ASDisplayNode()
        self.lockClickableNode.isUserInteractionEnabled = true
        
        super.init()
        
        self.addSubnode(self.clippingNode)
        self.clippingNode.addSubnode(self.lockClickableNode)
        ...
        self.lockClickableNode.isUserInteractionEnabled = true
//        self.lockClickableNode.backgroundColor = .red
        self.lockClickableNode.view.addGestureRecognizer(UITapGestureRecognizer(target: self, action: #selector(self.navigationTap)))
    }
    ...
    func updateLayout(size: CGSize, defaultHeight: CGFloat, leftInset: CGFloat, rightInset: CGFloat, transition: ContainedViewLayoutTransition) {
        ...
        self.lockClickableNode.frame = CGRect(origin: CGPoint(), size: size)
        ...
        transition.updateFrame(node: self.stripeNode, frame: CGRect(x: 0.0, y: size.height, width: size.width, height: CCScreenPixel))
