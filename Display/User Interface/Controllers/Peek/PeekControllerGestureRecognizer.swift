...
public final class PeekControllerGestureRecognizer: UIPanGestureRecognizer {
    ...
    private var longTapTimer: SSignalKit.Timer?
    private var pressTimer: SSignalKit.Timer?
    ...
    private func startLongTapTimer() {
        self.longTapTimer?.invalidate()
        let longTapTimer = SSignalKit.Timer(timeout: 0.4, repeat: false, completion: { [weak self] in
    ...
    private func startPressTimer() {
        self.pressTimer?.invalidate()
        let pressTimer = SSignalKit.Timer(timeout: 1.0, repeat: false, completion: { [weak self] in
