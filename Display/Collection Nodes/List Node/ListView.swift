...
open class ListView: ASDisplayNode, UIScrollViewAccessibilityDelegate, UIGestureRecognizerDelegate {
    ...
    private var itemReorderingTimer: SSignalKit.Timer?
    ...
    private func checkItemReordering(force: Bool = false) {
        ...
        if !force {
            self.itemReorderingTimer = SSignalKit.Timer(timeout: 0.025, repeat: false, completion: { [weak self] in
    ...
    public func transferVelocity(_ velocity: CGFloat) {
        ...
        self.decelerationAnimator = ConstantDisplayLinkAnimator(update: { [weak self] in
            ...
            let remainder = abs(strongSelf.accumulatedTransferVelocityOffset).remainder(dividingBy: CCScreenPixel)
    ...
    public func updateAccessoryNodes(transition: ContainedViewLayoutTransition, synchronous: Bool, currentTimestamp: Double, leftInset: CGFloat, rightInset: CGFloat) {
